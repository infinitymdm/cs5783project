...
After training, we sampled the fine-tuned models to evaluate their performance. The generated
samples are displayed in @fig-ldm1-samples. Based on the results we suspect that there was a
problem in the training process that we were unable to identify within the project timeline.
...
#figure(
    ...
    caption: [4 class-conditioned samples from the fine-tuned `ldm1` model.]
) <fig-ldm1-samples>
